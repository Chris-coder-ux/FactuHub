...
import { reconcileTransactions } from '@/lib/banking/matching';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify user has permission to reconcile transactions
    ...
    const { bankAccountId, periodStart, periodEnd } = await request.json();
    ...
    const result = await reconcileTransactions(bankAccountId, session.user.id, startDate, endDate);
